...
def start():
    ...
    cap = cv2.VideoCapture(0)
    recognized_users = set()  # Track who has been marked for attendance
    frame_counter = 0  # Safety measure to auto-stop after some time if no one is recognized
    ...
    while True:
        ...
        # Break if a user is recognized or ESC key is pressed
        if len(recognized_users) > 0 or cv2.waitKey(1) == 27 or frame_counter > 200:
            break
